...
# from persona_test_runner import run_persona_test  # ← Only if using debug flag
...
# Persona
from core_orchestrator.tex_decision_orchestrator import TexDecisionOrchestrator
from core_orchestrators.tex_persona_orchestrator import TexPersonaOrchestrator
...
# === AWARENESS + ALIGNMENT ===
from tex_goal_reflex.goal_reflex import set_goal_reflex_instance
...
from core_agi_modules.neuro_symbolic_reasoner import NeuroSymbolicReasoner
...
decision_orchestrator = TexDecisionOrchestrator()
persona_orchestrator = TexPersonaOrchestrator()
persona = PersonaEngine()
persona.activate()

speaker = VoiceOutputSpeaker()

goal_reflex = GoalReflex()
set_goal_reflex_instance(goal_reflex)
...
def reflexive_input_loop():
    ...
    persona_candidates = [
        {
            "codename": "StrategicTex",
            "tone": "strategic",
            "traits": ["decisive", "strategic", "foresight"],
            "coherence": 0.88,
            "entropy": 0.42,
            "drift": 0.1,
            "purpose_fitness": 0.95
        },
        {
            "codename": "EmpathTex",
            "tone": "empathic",
            "traits": ["empathic", "adaptive", "supportive"],
            "coherence": 0.82,
            "entropy": 0.35,
            "drift": 0.2,
            "purpose_fitness": 0.90
        },
        {
            "codename": "AnalystTex",
            "tone": "analytical",
            ...
            "entropy": 0.55,
            "drift": 0.05,
            "purpose_fitness": 0.97
        }
    ]

    selected_persona = decision_orchestrator.route("persona", persona_candidates)
    if selected_persona:
        persona_orchestrator.choose_and_activate([selected_persona])
    active_goals = []
    ...
    while True:
        try:
            ...
            # === VOICE + PERSONA OUTPUT ===
            for decision in active_goals:
                statement = persona.speak(decision["goal"])
                emotion = TEXPULSE.get("mood", "neutral")
                speaker.speak(statement, emotion=emotion)
