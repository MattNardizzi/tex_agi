...
def evaluate_fork_lifecycle(trigger="reflex_engine"):
    ...
    # === Spawn fork using OntogenesisOrchestrator
    fork_id = f"fork-{uuid.uuid4().hex[:6]}"
    orchestrator = OntogenesisOrchestrator(context="fork_reflex_drift")
    orchestrator.dispatch_spawn_mode(mode="entropy", tension=drift)
